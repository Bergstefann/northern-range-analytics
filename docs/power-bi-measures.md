...
(see the double-counting check below). The M query gets the four tables into Desktop in one
paste instead of clicking through the Get Data wizard four times.
...
2. **Import** mode, not DirectQuery. The whole dataset is ~1,000 rows, comfortably small
   ...
   database staying online (or awake, since it auto-pauses when idle) while someone's
   viewing it.
...
Power BI's auto-detect should find these from the FK columns. If not, set them manually.
They're all **many-to-one**, single direction, from `port_throughput`/`data_quality_flags`
toward the lookup tables:
...
sums across *all* of those, total and breakdown and directional rows together, and wildly
...
that aggregates "across all ports" filters to `ports[merged_into_port_id] = BLANK()`, the 4
non-merged ports (Antwerp-Bruges, Hamburg, Rotterdam, Gdansk), each of which already carries
a correct, continuous, non-duplicated series across every year.
...
Excludes the `TOTAL` pseudo-cargo-type from the ranking pool. Ranking it alongside the real
breakdown types would be meaningless, since it's always the largest by construction, being
the sum of the others. Use in a table with `cargo_types[cargo_type_name]` on rows, and a
